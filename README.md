...
| --- | --- |
| ![Карта и маршруты](Media/screenshot-map.png) | ![Гараж и улучшения](Media/screenshot-garage.png) |
...
За успешную доставку начисляются кредиты и репутация. Награда считается по формуле: базовые 30 кредитов, половина расстояния от базы до точки назначения и 30% от веса груза. Если отказаться от заказа, компания потеряет 10% его репутации, но не меньше одного очка. Просроченный заказ отнимает всю обещанную репутацию. При падении репутации ниже `-100` игра заканчивается.

Новые заказы появляются постепенно. В начале они легче и подходят стартовым роверам, а затем становятся тяжелее и разнообразнее. Если все роверы свободны и активных задач нет, игра быстрее предлагает следующий заказ.
...
Для WebGL-сборки понадобится модуль WebGL Build Support. Профиль сборки выбирается в окне Build Profiles. Готовую версию нужно запускать через локальный HTTP-сервер или разместить на хостинге: Unity WebGL не работает при открытии `index.html` напрямую через `file://`.
...
- Suno AI использовался для создания фоновой ambient-композиции.
